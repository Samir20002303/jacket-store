...
export default function CartPage() {
  const { cart, totalCartItems, updateQuantity, removeFromCart, getProductStock } = useStore();

  const subtotal = cart.reduce((sum, item) => sum + item.price * item.quantity, 0);
  const shipping = subtotal > 150 ? 0 : 15;
  const total = subtotal + shipping;

  const { user } = useAuth(); // ← AJOUTER l'import
  const [authModalOpen, setAuthModalOpen] = useState(false); 

  if (cart.length === 0) {
    return (
      <StandardPageShell title="Cart" description="Your cart is empty — time to explore.">
        <div className="flex flex-col items-center justify-center gap-4 py-20">
          ...
          <Link href="/shop" className="mt-2 rounded-full bg-black px-6 py-3 text-sm font-semibold text-white transition hover:scale-[1.02] active:scale-[0.98]">
            Explore shop
          </Link>
        </div>
      </StandardPageShell>
  ...
  return (
    <>
    <StandardPageShell title={`Cart (${totalCartItems})`} description="Review your selection before checkout.">
      <div className="grid gap-8 lg:grid-cols-[1fr_380px]">
        <div className="space-y-3">
          {cart.map((item) => {
            const stockRestant = getProductStock(item.productId, item.size as Size);
            const maxAvailable = stockRestant + item.quantity;
              ...
              >
                <div className="relative h-20 w-20 flex-shrink-0 rounded-xl bg-black/[0.03]">
                  {item.image ? (
                    ...
                    <div className="grid h-full w-full place-items-center text-xs text-black/30">No img</div>
                  )}
                </div>

                <div className="flex-1 min-w-0">
                  <h3 className="font-semibold text-black truncate">{item.name}</h3>
                  <p className="text-sm text-black/50">{item.color} — Size {item.size}</p>
                  <p className="mt-1 text-sm font-medium text-black">${item.price}</p>
                </div>
                ...
                <div className="flex flex-col items-end gap-2">
                  <div className="flex items-center rounded-full border border-black/15 bg-black/[0.02]">
                    <button
                      type="button"
                      onClick={() => updateQuantity(item.productId, item.size as Size, item.quantity - 1)}
                    ...
                    <button
                      ...
                      className="grid h-8 w-8 place-items-center text-sm text-black/50 transition hover:text-black disabled:opacity-30 disabled:cursor-not-allowed"
                      aria-label="Increase"
                    >
                      <svg className="h-3 w-3" viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="h-fit rounded-2xl border border-black/8 bg-white p-6">
          ...
          <div className="mt-4 space-y-3 text-sm">
            <div className="flex justify-between text-black/70">
              <span>Subtotal</span><span className="font-medium tabular-nums">${subtotal.toFixed(0)}</span>
            </div>
            <div className="flex justify-between text-black/70">
              <span>Shipping</span>
              <span className="font-medium tabular-nums">
                {shipping === 0 ? <span className="text-green-600">Free</span> : `$${shipping.toFixed(0)}`}
              </span>
            </div>
            {shipping > 0 && <p className="text-xs text-black/40">Free shipping on orders over $150</p>}
            <hr className="border-black/10" />
            <div className="flex justify-between text-base font-semibold text-black">
              <span>Total</span><span className="tabular-nums">${total.toFixed(0)}</span>
            </div>
          </div>
          <button
            type="button"
          ...
          <Link href="/shop" className="mt-3 block text-center text-sm text-black/50 underline underline-offset-2 transition hover:text-black">
            Continue shopping
          </Link>
        </div>
      </div>
